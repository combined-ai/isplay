...
export class AnalysisResource extends BaseResource {
  ...
  async createExperiment(input: CreateExperimentInput): Promise<CreateExperimentResponse> {
    return this.transport.unwrap(await this.transport.client.POST("/v1/experiments", { body: input }), "POST", "/v1/experiments");
  }

  async createHypothesisBatch(input: CreateHypothesisBatchInput): Promise<HypothesisBatchCreateResponse> {
  ...
  async runExperiment(id: string, input: RunExperimentInput): Promise<ExperimentRunResponse> {
  ...
  async experimentResults(id: string, page?: PageQuery): Promise<ExperimentResultsResponse> {
    return this.transport.unwrap(await this.transport.client.GET("/v1/experiments/{id}/results", { params: { path: { id }, query: page } }), "GET", "/v1/experiments/{id}/results");
  }

  async experimentRequirements(id: string, page?: PageQuery): Promise<FixtureRequirement[]> {
    const result = await this.transport.client.GET("/v1/experiments/{id}/requirements", { params: { path: { id }, query: page } });
    return this.transport.unwrap(result, "GET", "/v1/experiments/{id}/requirements");
  }

  async experimentTrialMatrix(id: string, page?: PageQuery): Promise<TrialMatrixRow[]> {
    const result = await this.transport.client.GET("/v1/experiments/{id}/trial-matrix", { params: { path: { id }, query: page } });
  ...
  async experimentArmComparison(id: string, page?: PageQuery): Promise<ArmComparisonRow[]> {
    const result = await this.transport.client.GET("/v1/experiments/{id}/arm-comparison", { params: { path: { id }, query: page } });
