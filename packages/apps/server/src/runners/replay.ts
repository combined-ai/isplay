...
import { nowIso, ReplaySchema, stableHash, type CreateAnalysisRunInput, type FixtureRequirement, type Replay, type ToolFixture } from "@isplay/core";
...
export async function executeReplay(store: IsplayStore, replay: Replay): Promise<Replay> {
  const running = await store.updateReplay(ReplaySchema.parse({ ...replay, status: "running", startedAt: replay.startedAt ?? nowIso(), endedAt: undefined, error: undefined }));
  const attempt = await startAttempt(store, running);
