...
Capture is observational. It records a baseline. Replay is counterfactual. It applies branch interventions and policies to ask "what would have changed if this input/state/tool result/config had been different?"
...
`hash`, `encrypt`, `artifact_only`, and `metadata_only` are policy actions, but current replacement output is intentionally conservative. `hash` emits `[HASHED]`; `encrypt`, `artifact_only`, and `metadata_only` mask as `[REDACTED]` rather than wiring encryption or alternate storage semantics.
</Callout>
