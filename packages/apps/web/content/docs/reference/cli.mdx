...
| `isplay jobs events <jobId>` | none | SSE text | Current server returns simple started/finished events. |
...
| `replay events <replayId>` | Fetch replay events, falling back to base run events if none were stored. |
...
`fixtures add` uses `provenance: "analyst_fixture"` and `author: process.env.USER`. If `--args-hash` is omitted, it looks for an open requirement on that replay and tool.
...
<Callout type="warning" title="No-wait caveat">
`experiments run --no-wait` is parsed by the CLI, but the current server route runs synchronously and returns results.
</Callout>
