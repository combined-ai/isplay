...
npx isplay branch intervene <branchId> \
  --project "$ISPLAY_PROJECT_ID" \
  --kind prompt_patch \
  --target <event-or-ref-id> \
  --patch patch.json \
  --description "Remove the duplicate receipt escalation clause"
```

CLI interventions target an event ID, `refId`, or event type. Experiment specs can target richer selectors such as `refId`, `eventId`, `eventType`, `toolName`, `modelCallId`, `artifactId`, or `jsonPointer`.
