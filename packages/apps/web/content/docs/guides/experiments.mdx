...
Runs current arms. The current server runner is synchronous and serial even though the schema includes `wait`, `concurrency`, and `maxReplays`.
...
- Treat `--no-wait` as future-facing: the current server route still returns after synchronous execution.
